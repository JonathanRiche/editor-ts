...
export class JsonContentAdapter implements ContentAdapter {
  ...
  async describeWorkspace(): Promise<ContentAdapterWorkspaceDescriptor> {
    ...
    const scriptPaths = Object.keys(files)
      .filter((path) => path.startsWith('components/') && path.endsWith('.js'))
      .sort((a, b) => a.localeCompare(b));
  ...
  async buildAiWorkspace(): Promise<ContentAdapterAiWorkspace> {
    ...
    return {
      files,
      editablePaths,
      readOnlyPaths: ['index.html'],
      mode: 'canonical',
      descriptor: await this.describeWorkspace(),
    };
  }

  async describePreview(): Promise<ContentAdapterPreviewDescriptor> {
    const workspace = await this.describeWorkspace();
