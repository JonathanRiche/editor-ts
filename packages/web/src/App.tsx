...
import {
  SHARED_AI_UI_IDS,
} from '@editorts/starter-shared/aiIds';
import {
...
export default function App() {
  ...
  const initializeWorkspace = async (args: {
    ...
    const workspaceDescriptor = await editor.content.describeWorkspace();
    const workspaceLabel = workspaceDescriptor
      ? workspaceDescriptor.runtime === 'app'
        ? ` Detected ${workspaceDescriptor.kind} app workspace. AI now uses project files directly; app-preview canvas wiring is still a separate next step.`
        : ` Detected ${workspaceDescriptor.kind} page workspace.`
      : '';

    setWorkspaceMode(args.mode);
    setFolderName(args.folderName ?? null);
    setStatusText(`${args.status}${workspaceLabel}`);
  ...
  onMount(() => {
    ...
    setAiBaseUrl(loadStoredAiBaseUrl(AI_BASE_URL_STORAGE_KEY, DEFAULT_AI_BASE_URL));
