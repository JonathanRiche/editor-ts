...
const editorConfig: InitConfig = {
  storage: {
    type: 'local',
    // For SQLocal + Vite, see packages/localsql.
    // type: 'sqlocal',
    // prefix: 'quickstart_',
  },
...
void editor.preview.describe().then((preview) => {
  if (!previewStatus) return;

  if (!preview) {
    previewStatus.textContent = 'Preview mode unavailable';
    return;
  }

  if (preview.mode === 'app-url') {
    previewStatus.textContent = `App preview via ${preview.baseUrl ?? preview.entryUrl ?? 'configured URL'}`;
    return;
  }

  previewStatus.textContent = 'Page preview via iframe srcdoc';
});
