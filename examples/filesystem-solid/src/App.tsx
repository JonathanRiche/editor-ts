...
import { createCustomComponentDefinition } from '../../../src/core/CustomComponentRegistry';
...
export default function App() {
  ...
  const onPickProject = async () => {
    ...
    try {
      ...
      await initializeWithProvider(fs, 'Browser folder connected. Files tab is live.');
    } catch (err: unknown) {
  ...
  const onConnectServerRoutes = async () => {
    ...
    try {
      ...
      await initializeWithProvider(fs, `Server routes connected: ${root}`);
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : String(err);
